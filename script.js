  ...
  try {
    ...
    const card = `
        <div class="head-container">
            ...
            <div class="right">
                <a href=${profileLink} target="_blank"><h3>${data.name}</h3></a>
    ...
    console.log("repodata", userRepoData);

    const repo = `
    <div class="repositories">
            ${userRepoData
              .map((item, index) => {
                repoLink = profileLink + item.name;
                return `
                <div class="repoItem"> 
                  <a href=${repoLink} target="_blank"><h3>${item.name}</h3></a>
